...
function handleOrientation(event) {
	...
	if(alpha && beta && gamma) {
		...
		try {
			var newTop = ((window.innerHeight / 2) - 320) - (((beta - betaOffset) / 90) * (window.innerHeight / 2));
			background.style.top = newTop;
			var newLeft = ((window.innerWidth / 2) - 320) - (((gamma - gammaOffset) / 90) * (window.innerWidth / 2));
